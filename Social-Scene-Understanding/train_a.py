...
from volleyball_loader_a import VolleyballDataset
import pickle
...
# __OLD_BACKBONE_PATH = './models/backbone/backbone lr-1e-05 weight_decay-4e-05 crop_size-5 embed_dim-2048 epoch-43 dropout_ratio-0.2.pth'
# __OLD_CLASSIFIER_PATH = './models/classifier/classifierA lr-1e-05 weight_decay-4e-05 crop_size-5 embed_dim-2048 epoch-43 dropout_ratio-0.2.pth'
__BONE_PATH = './models/backbone-atten/'
__CLASSIFIER_PATH = './models/classifier-atten/'
...
__LOG_PATH = './log/A-atten lr-{} weight_decay-{} crop_size-{} embed_dim-{} drpout_ratio-{}.txt'.format(
    __LEARNING_RATE, __WEIGHT_DECAY, __CROP_SIZE, __EMBED_DIM, __DROPOUT_RATIO)
        ...
        for phase in phases:
            ...
            for img_tensor, bboxes, actions, activities, group_info in dataloaders_dict[
                ...
                with torch.set_grad_enabled(phase == 'trainval'):
                    ...
                    mixed_5d, mixed_6e = backbone(img_tensor)
                    ...
                    action_logits, activity_logits, _ = classifier(
            ...
            if (epoch + 1) % __STORE_GAP == 0:
                torch.save(
                    ...
                    'backbone-atten lr-{} weight_decay-{} crop_size-{} embed_dim-{} epoch-{} dropout_ratio-{}.pth'
                ...
                torch.save(
                    ...
                    'classifierA-atten lr-{} weight_decay-{} crop_size-{} embed_dim-{} epoch-{} dropout_ratio-{}.pth'
                    .format(__LEARNING_RATE, __WEIGHT_DECAY, __CROP_SIZE,
                            __EMBED_DIM, epoch + 1, __DROPOUT_RATIO))
