...
class ClassifierA(nn.Module):
    ...
    def forward(self, feature):
        ...
        if (len(feature_embed.shape) == 3):
            activity_logits = self.predict_activity(
                feature_embed.unsqueeze(dim=1))
            # [B, 12, 2048] -> [B, 1, 12, 2048] -> [B, 1, 1, 2048] -> [B, 1, 1, 8]
        else:
            activity_logits = self.predict_activity(feature_embed)
            # [B, T, 12, 2048] -> [B, T, 1, 2048] -> [B, T, 1, 8]
        activity_logits = activity_logits.view(-1, self.activity_dim)
        # [B, 1, 1, 8] -> [B, 8]
        return action_logits, activity_logits, feature_embed
...
class ClassifierB(nn.Module):
    ...
    def forward(self, feature):
        ...
        action_logits = self.predict_action(total_hiddens).view(
            -1, self.action_dim)
        # [B, T, N, 1024] -> [B, T, N, 9] -> [BTN, 9]
