    ...
    for frame in frames:
        ...
        for occupant in frame_occupants:
            ...
            for occu in frame_occupants:
                if frame_ptr > 0 and occu in hidden_states[frame_ptr-1] and occu != occupant:
                    nei_hidden.append(hidden_states[frame_ptr-1][occu].view(1, __HIDDEN_DIM))
            ...
            # get hiddens
            predict, hidden = model(position, prev_hidden, spat_hidden, prev_hidden)
            ...
            if len(path) > __OBSERVE_LEN+1:
                ...
                prev_hidden = hidden_states[frame_ptr-__OBSERVE_LEN-1][occupant]
                ...
                for i in range(__OBSERVE_LEN):
                    ...
                    for occu in occu_dict[frame_ptr - __OBSERVE_LEN + i]:
                        if occu != occupant:
                            van_hid.append(hidden_states[frame_ptr - __OBSERVE_LEN + i][occu].view(1, __HIDDEN_DIM))

                    van_spat_hid = torch.cat(van_hid)
                    if torch.cuda.is_available():
                        van_spat_hid = van_spat_hid.cuda()

                    van_temp_hid = hidden_states[frame_ptr - __OBSERVE_LEN + i][occupant]

                    predict, hidden = model(observe_xy[i], van_temp_hid, van_spat_hid, prev_hidden)
                    prev_hidden = hidden
    ...
    if (epoch + 1) % 3 == 0:
        for occupant in occupants:
            ...
            if len(real_x[occupant]) > 0:
                ...
                plt.savefig('images/epoch{}_scene{}_occupant{}_{}_{}_real.jpg'
                    .format(epoch+1, s+1, occupant, len(real_x[occupant]), c.lower()))
                plt.clf()
                plt.plot(predict_x[occupant], predict_y[occupant], color='blue',linewidth=2, alpha=0.5)
                plt.savefig('images/epoch{}_scene{}_occupant{}_{}_{}_predict.jpg'
                    .format(epoch+1, s+1, occupant, len(real_x[occupant]), c.lower()))
...
def train_with_attn(models, num_scenes, learning_rates, num_epochs, evaluate_loss_after=1):
    prev_cost = {c: float("inf") for c in classes}
    training_set = TrainDataset()
    training_generator = DataLoader(dataset=training_set, batch_size=1, shuffle=False, collate_fn=collate_fn)
    ...
    for epoch in range(num_epochs):
        ...
        for i in training_generator:
            ...
            if (epoch + 1) % evaluate_loss_after == 0:
                cost = step_through_scene(models, scene, epoch, num_epochs, s, True)
                ...
                for c in classes:
                    ...
                    plt.savefig('costs/epoch{}_scene{}_{}.jpg'.format(epoch+1, s+1, c.lower()))

                    f = open('log.txt','a')
                    f.write('epoch{}_scene{}_{}\'cost:{}\n'.format(epoch+1, s+1, c.lower(), cost[c]))
                    f.close()
...
models = {label: AttnGRU(__INPUT_DIM, __OUTPUT_DIM, __HIDDEN_DIM, __ATTN_DIM) for label in classes}
if torch.cuda.is_available():
    for c in models:
