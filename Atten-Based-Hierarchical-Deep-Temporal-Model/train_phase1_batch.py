...
__OLD_VGG_PATH = './vgg_models/checkpoint125.pth'
__MODEL_PATH = './models/checkpoint_{}_{}_vgg129.pth'.format(
    __GRU_ACTIVE, __HIDDEN_DIM)
__VGG_PATH = './vgg_models/checkpoint129.pth'

__LOG_PATH = './log/{}_{}_vgg.txt'.format(__GRU_ACTIVE, __HIDDEN_DIM)
note = "with vgg {}".format(
...
def collate_fn(batch):
    ...
    for pixel, action, info in zip(person_pixels, person_actions, group_info):
        pixels += pixel
        actions += action
        infoes += info
        num_per_frame.append(len(action))

    return pixels, actions, infoes, num_per_frame
...
def train_model(model, vgg, extractor, dataloaders_dict, criterion, optimizer):
    ...
    for epoch in range(__NUM_EPOCHS):
        ...
        for phase in ['train', 'val']:
            ...
            # Iterate over data.
            for inputs, person_actions, group_info, num_per_frame in dataloaders_dict[
                    phase]:
                print(group_info)
                person_num = len(person_actions)
                pidxes = range(person_num)

                person_features = {fidx: 0 for fidx in range(__FRAME_LEN)}

                # ============= fine tune vgg and extract person_features =============
                new_action = []
                ...
                for fidx in range(__FRAME_LEN):
                    # ptr = pidx * __FRAME_LEN
                    pixel = torch.from_numpy(np.stack(inputs[fidx::10],
                                                      axis=0)).to(device)
                    action = torch.tensor(new_action[fidx::10]).to(device)
                    ...
                    # === init extractor to get the extracted person_features ===
                    ...
                    with torch.set_grad_enabled(False):
                        ...
                        # print("Forward the extractor")
                        feature = extractor(pixel)  # (batch_size, 4096)
                        # print(feature.shape)

                        person_features[fidx] = feature

                # for pidx in pidxes:
                #     pidx_ptr = pidx * __FRAME_LEN
                #     pixel = np.stack(
                #         inputs[pidx_ptr:pidx_ptr + __FRAME_LEN], axis=0)
                ...
                # print('VGG|{}|{}|'.format(
                #     time.strftime('%m-%d %H:%M:%S', time.localtime(vgg_since)),
                #     time.strftime('%m-%d %H:%M:%S',
                #                   time.localtime(time.time()))))

                # ========================== begin Reactive LSTM training ==========================
                # print("Begin Reactive LSTM training")
                hidden_states = {fidx: 0 for fidx in range(__FRAME_LEN)}
                # for fidx in range(__FRAME_LEN):
                #     person_features[fidx] = torch.stack(features[fidx]).to(device)

                nei_ptr = 0
                nei_group = []
                # nei_mask = []
                for num in num_per_frame:
                    nei_group.append(pidxes[nei_ptr:nei_ptr + num])
                    # for i in range(num):
                    #     nei_mask.append(1)
                    # for i in range(__PERSON_NUM - num):
                    #     nei_mask.append(0)
                    nei_ptr += num
                    if nei_ptr == person_num:
                        break

                # since_iter1 = time.time()
                # ================= 1st iteration to get the hiddens =================
                # print("1st iteration to get the hiddens")
                for fidx in range(__FRAME_LEN):
                    nei_hiddens = []
                    if fidx == 0:
                        # initialization
                        ...
                        for pidx in pidxes:
                            # pid_ptr = pidx // __PERSON_NUM
                            # nei_idxes = range(pid_ptr, pid_ptr + __PERSON_NUM)
                            frame_person_num = 0
                            for group in nei_group:
                                if pidx in group:
                                    nei_idxes = copy.copy(group)
                                    frame_person_num = len(group)
                                    break
                            ...
                            for nei_idx in nei_idxes:
                                temp_hidden.append(
                                    hidden_states[fidx - 1][nei_idx].view(
                                        1, -1))
                            # print(frame_person_num)
                            for i in range(__PERSON_NUM - frame_person_num):
                                temp_hidden.append(
                                    torch.zeros(1, __HIDDEN_DIM).to(device))
                            # print(torch.cat(temp_hidden).to(device).shape)
                            nei_hiddens.append(
                                torch.cat(temp_hidden).to(device))

                        # print(torch.stack(nei_hiddens).shape)
                ...
                # since_iter2 = time.time()
                # ============== 2nd iteration to train the model ==============
                # print("2nd iteration to train the model")
                person_loss = 0.0
            ...
            print('{} {} LSTM Loss: {:.4f} Acc: {:.4f} '.format(
